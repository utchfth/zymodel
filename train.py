...
train_data_dir = '../autodl-tmp/data/train/ITS/'
...
if category == 'indoor':
    val_data_dir = '../autodl-tmp/data/test/SOTS/indoor/'
elif category == 'outdoor':
    val_data_dir = '../autodl-tmp/data/test/SOTS/outdoor/'
else:
...
for epoch in range(1 + opt.continueEpochs, opt.nEpochs + 1 + opt.continueEpochs):
    ...
    for iteration, inputs in enumerate(train_dataloader,1):
        ...
        # 感知损失
        creation= PerceptualLoss(loss_func, [3, 8, 15, 22], device)
        perceptual_loss = creation(dehaze, gt)

        Loss = MSE_loss +0.01*EDGE_loss+0.01*perceptual_loss
        epoch_loss +=Loss
        Loss.backward()
        optimizer.step()

        if iteration % 100 == 0:
            print("===>Epoch[{}]({}/{}): Loss: {:.4f} MSELoss: {:.4f} EDGELoss: {:.4f} Perceptual_loss：{:.4f}".format(epoch, iteration, len(train_dataloader), Loss.item(), MSE_loss.item(), EDGE_loss.item() ,perceptual_loss()))
